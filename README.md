...
- **A state provider**, an async closure from the remote peer's endpoint id to `OutboundState` — the current seq plus the envelope items to send when that peer is behind. It's called once per contact, so seq and payloads are read as one consistent snapshot, and it sees who's asking so an app can shape what it serves per peer — the same state for everyone (ignore the argument), or different slices for different relationships, down to nothing at all for a peer it doesn't recognize.
